...
class Player(AbstractPlayer):
    ...
    def build_graph_from_state(state: PlayerState):
        ...
        # removing the blocked cells from the graph
        for row in range(num_rows):
            for col in range(num_cols):
                loc = row, col
                if loc not in [state.my_loc, state.rival_loc] and board[loc] == BLOCK_CELL:
                    g.remove_node(loc)
    ...
    @staticmethod
    def dfs_get_longest_path(g: nx.Graph, src):
        successors = nx.dfs_successors(g, src)

        return Player.recursive_get_longest_path(successors, src)

    @staticmethod
    def recursive_get_longest_path(node_to_successors: dict, src):
        if src not in node_to_successors:
            return 0

        successors = node_to_successors[src]

        list_of_successor_path_length = []

        for successor in successors:
            list_of_successor_path_length.append(1 + Player.recursive_get_longest_path(node_to_successors, successor))
