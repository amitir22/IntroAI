    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                #   (i)   the `heuristic_weight` set to the mid point of the current low & high binary search bound
                mean_heuristic_weight = (high_heuristic_weight + low_heuristic_weight) / 2

                current_a_star = AStar(heuristic_function_type=self.heuristic_function_type,
                                       heuristic_weight=mean_heuristic_weight,
                                       max_nr_states_to_expand=self.max_nr_states_to_expand_per_iteration)
                current_res = current_a_star.solve_problem(problem)

                #  Don't forget to update `total_nr_expanded_states` and `max_nr_stored_states` (see how we've done
                #   it above).
                total_nr_expanded_states += current_res.nr_expanded_states
                max_nr_stored_states = max(max_nr_stored_states, current_res.max_nr_stored_states)

                # "a solution has been found for this value of w"
                if current_res.is_solution_found:
                    if current_res.solution_g_cost < best_solution.solution_g_cost:
                        best_solution = current_res
                        best_heuristic_weight = mean_heuristic_weight
                    high_heuristic_weight = mean_heuristic_weight
                else:
                    # "a solution could not be found (under the #expanded-states constrains)
                    # for this value of w and for lower ones"
                    low_heuristic_weight = mean_heuristic_weight
